...
| `tsc --noEmit` 类型检查 | ✅ **零错误** |
| `npm test` 冒烟测试 | ✅ **173 项全通过**（验签/解密、命令解析、去重限流、webhook 端到端、回合流程、会话压缩、云文档授权与读写工具族、工作区仓库、跨会话记忆） |
...
│       ├── docs.ts        ➕ 新增   # 飞书云文档七个工具（读 3 + 写 3 + 建 1，应用身份优先）
│       ├── event.ts       ✅ 原样   # 事件体解析
...
│
├── test/
│   └── smoke.ts                    # 冒烟测试：173 项，裸 Node 跑，不依赖 EdgeOne
│
└── docs/
