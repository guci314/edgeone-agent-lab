...
export const SUMMARY_SYSTEM_PROMPT = [
  ...
  "   · 引用过的来源：网页 URL、飞书云文档链接，**原样保留**（后续要能顺着追溯）；",
  "   · 讨论过的问题和得出的关键结论。算出来的数字要连口径一起留（单位、时间范围、数据来源）；",
  "   · 用户表达过的偏好或决定（例如「只看某一段」「别用某个数据源」「这个数字才对」）；",
  "   · 还没做完的事、悬而未决的问题。",
 ...
 * 超长字符串保留头尾。
 *
 * 只留头会把「用户最后强调的约束」丢掉，只留尾会把开头那句「用户到底要干什么」
 ...
 * 用户第一次把「要干什么、有什么约束」讲清楚的那一轮，后面几十轮都是围着它往返。
 * 只保留尾部的话，摘要会丢掉那个出发点 —— 后面的追问全都建立在它之上，
 * 丢了就变成一堆没有主语的结论。
 */
